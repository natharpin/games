...
public class Driver extends Application {
	...
	void initialize()
	{
		turret = new Javelin(new Point(WindowSettings.WIDTH / 2, WindowSettings.HEIGHT / 2), new Point(WindowSettings.WIDTH / 2, WindowSettings.HEIGHT / 2));
	...
	private void spawnTargets(){
		...
		for(int i = 0; i < numTargets; i++){
			targets.add(new Target(new Point(rng.nextInt(WindowSettings.WIDTH - 30) + 15, rng.nextInt(WindowSettings.HEIGHT - 30) + 15), new Vector(new Point(), randomSpeedPoint(targetSpeed))));
		}
		round++;
	}
	...
	public void start(Stage theStage) {
		...
		KeyFrame kf = new KeyFrame(Duration.millis(1000 / WindowSettings.FPS),
				e -> {
					// update position
					update();
					// draw frame
